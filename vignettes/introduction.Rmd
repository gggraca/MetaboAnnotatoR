...
The following function will place the example 'targetTable.csv' and 'XCMS_options.csv' files as described above in the user working directory:
```{r load_data, eval=TRUE, results='hide'}
getDemoData()
```

```{r remove_files, eval=TRUE, echo=FALSE}
file.remove("targetTable.csv")
file.remove("XCMS_options.csv")
```

An example dataset used consisting of a lipidomics dataset (reverse-phase chromatography, containing both lipids and lipid-like molecules) acquired in ESI-MS positive mode should be downloaded from [zenodo repository](https://zenodo.org/records/17408169) and placed in the working directory.
...
```{r metabolites_example, eval=FALSE}
annotateAIF(targetTable = "targetTable.csv", 
  filetype = "mzML", 
  libs = "Metabolites",
  ESImode = "POS",
