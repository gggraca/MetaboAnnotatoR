### Function annotateAgilentAIF for raw LC-MS chromatogram                                                              
### Search possible matches of a feature in Lipid fragments and other small molecule libraries                     
### using 'raw' LC-MS single chromatogram information already loaded and peak-picked                               
### returns a ranked list of candidates for the feature annotation as .csv file
### Arguments:
### targetTable: a .csv file containing the list of features to annotate and the name of the files containing the raw data
### ESImode: ionization mode: 'POS' for positive (default) or 'NEG' for negative                                   
### libs: fragments library: 'Lipids' (default) or 'Metabolites' for other small molecules
...
### checkIsotope: whether or not to check the isotope type; default is to check (TRUE)                             
### tolerance: tolerance in ppm for the candidate search
### maxMZdiff: maximum m/z difference between candidate fragments and pseudo-MS/MS or AIF ions                                                                   
### Goncalo Graca & Yuheng Cai (Imperial College London)
### g.gomes-da-graca@imperial.ac.uk
### this version is for Agilent AIF containing 2 or more collision energies
### only one collision energy (30 eV in this case) will be used for pseudo-MS/MS extraction
### 6 March 2021

annotateAgilentAIF <- function(targetTable, filetype = "mzML", libs = "Lipids", ESImode = "POS", RTfile = "none", corThresh = 0.7,
					 checkIsotope = TRUE, tolerance = 25, maxMZdiff = 0.01, matchWeight = 0.5, ncandidates = 5){
...
libfiles <- list.files(path = paste("./Libraries/",libs,"/", ESImode, sep = ""), full.names = TRUE)
...
if(length(unique(targets[,3])) == 1){
  ...
  xcmsF2@featureData@data <- xcmsF2@featureData@data[which(xcmsF2@featureData@data$collisionEnergy == 30),]
  xcmsF2@featureData@data$msLevel <- 1 # this is necessary to enable xcms to read from MS2 scans
  # try to use same centwave parameters for both no- and high-collision scans
  cwp <- CentWaveParam(snthresh = xcmsOptions[3,2], 
                        noise = xcmsOptions[2,2], 
                        ppm = xcmsOptions[1,2], 
                        peakwidth = as.numeric(xcmsOptions[4,2:3]),
                        prefilter = as.numeric(xcmsOptions[5,2:3]))
  peaksF1 <- findChromPeaks(xcmsF1, param = cwp)
  peaksF2 <- findChromPeaks(xcmsF2, param = cwp)
} else NULL


for (i in 1:dim(targets)[1]){
	message(paste("####### Processing feature", i, "of", dim(targets)[1]), " ########")	
	if(length(unique(targets[,3]))>1){
		message("Reading data...")
	...
	Sp.idx <- length(Sp[[1]])
	SpName <- Sp[[1]][Sp.idx]
	SpName <- gsub(".mzML", "", SpName)
	
	# get MS spectra at feature RT --------------------
	message("Obtaining pseudo-MS/MS spectrum...")
	try(
	  specs <- getPseudoMSMS(fmz, frt, xcmsF1, xcmsF2, peaksF1, peaksF2, filetype = "mzML", 
	                         CE = 2, cthres1 = corThresh, cthres2 = corThresh, 
	                         plotResults = TRUE, SpName, DirPath)
	)
	...
	candidates <- searchLib(lib, libfiles, fmz - iso * 1.0034, frt, tolerance = tolerance, RTs, inSourceSpec)
	}
	
	# Compare fragments between Library candidates and high-collision-energy and pseudo-MS/MS spectra --------------------
	if(is.null(pseudoSpec) & is.null(highCESpec) | length(unlist(candidates)) == 0) {
	  result <- NULL
	} else {
	message("Comparing pseudo MS/MS and high collision energy MS with candidate(s) fragments...")
	output <- mapply(compFrag, candidates, lapply(as.numeric(names(candidates)), function(x) lib[[x]]),
	                     MoreArgs = list(fmz, frt, iso, highCESpec, pseudoSpec, tolerance = tolerance,
						 maxMZdiff = maxMZdiff, matchWeight = matchWeight), SIMPLIFY = FALSE)
	...
	rankedResult[c("metabolite", "feature.type", "ion.type", "isotope", "mz.metabolite", "matched.mz",
	...
	if(is.null(result)) {
	write.csv(rankedResult, file = paste(mainDir, "/", subDir,"/", SpName,"_", round(fmz,3),"mz_",
	          round(frt,3),"s_","ranked_candidates.csv",sep = ""), row.names = FALSE)
	} else {
	write.csv(rankedResult[rankedResult$rank <= ncandidates,], 
	          file = paste(mainDir, "/", subDir,"/", SpName,"_", round(fmz,3),"mz_",
	          round(frt,3),"s_","ranked_candidates.csv",sep = ""), row.names = FALSE)
	}
	...
	global[i,"score"] <- rankedResult[1,"score"]
	  ...
	  global[i,c("metabolite", "feature.type", "ion.type", "isotope", "mz.metabolite", 
	             "matched.mz", "mz.error", "pseudoMSMS", "fraction", "score")] <- NA
	}
	# save image with matched spectra and EICs
	# must update the folder to save images in...can be updated to work with single feature
	if (exists('rankedSpec') & !is.null(result)){
	  if (length(rankedSpec) == 1) plotCandidatesAIF(fmz, frt, highCESpec, ms2eic, SpName, output, 1, DirPath)
	  if (length(rankedSpec) <= ncandidates) {
	    #try(
	      plots <- lapply(1:length(rankedSpec), function(x) plotCandidatesAIF(fmz, frt, highCESpec, ms2eic, SpName, output, x, DirPath))
	    #)
	  }
	  if (length(rankedSpec) > ncandidates) {
	    #try(
	    plots <- lapply(1:length(rankedSpec[1:ncandidates]), function(x) plotCandidatesAIF(fmz, frt, highCESpec, ms2eic, SpName, output, x, DirPath))
	    #)
	  }
	}	else NULL
  }   
    # save global results table
   write.csv(global, file = paste(mainDir, "/", subDir, "/", "Global_Results", ".csv", sep = ""), row.names = FALSE)
	# save general options
	df <- data.frame(targetsTable_file = targetTable, libraries = libs, ESImode = ESImode, 
                     RTfile = RTfile, corThresh = corThresh, checkIsotope = checkIsotope, 
					 matchWeight = matchWeight, tolerance = paste(tolerance, "ppm"), 
					 maxMZdiff = paste(maxMZdiff, "Da"), row.names = "Option")
	df <- as.data.frame(t(df))				 
	write.csv(df, file = paste(mainDir, "/", subDir, "/", "General_options", ".csv", sep = ""))
	# save xcms options
	write.csv(xcmsOptions, file = paste(mainDir, "/", subDir, "/", "XCMS_options", ".csv", sep = ""), row.names = FALSE)
message('Job done!')
}