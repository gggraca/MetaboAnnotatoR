...
### variable name change "MZerr" to "maxMZdiff" 
### 30/11/2020

annotateRC <- function(targetTable, xcmsObject, ramclustObj, libs = "Lipids", ESImode = "POS", RTfile = "none",
...
global <- targets
global[,c("metabolite", "feature.type", "ion.type", "isotope", "mz.metabolite", "matched.mz",
...
lib <- sapply(libfiles, read.csv, header = TRUE, sep = ",", check.names = FALSE) # check.names=FALSE to use the original header names in the annotations
...
Time <- gsub(":", "_", Time)
subDir <- paste(libs, "_", ESImode, "_", "RAMClustR", "_", Date, "_", Time, sep = "")
...
for (i in 1:dim(targets)[1]){
	...
	rankedResult[c("metabolite", "feature.type", "ion.type", "isotope", "mz.metabolite", 
	...
	write.csv(rankedResult[rankedResult$rank <= 5,], file = paste(mainDir, "/", subDir, "/", DatasetName, 
	"_", round(fmz,3), "mz_", round(frt,3), "s_", "ranked_candidates.csv" , sep = ""), row.names = FALSE)
	...
	if (exists("rankedSpec") & !is.null(result)){
	  ...
	  plotCandidatesRC(fmz, frt, highCESpec, DatasetName, output, 1, DirPath = paste(mainDir,"/",subDir, "/", sep = ""))
	  }
	  if (length(rankedSpec) <= ncandidates) {
	    #try(
	      plots <- lapply(1:length(rankedSpec), function(x) plotCandidatesRC(fmz, frt, highCESpec, DatasetName,
		  output, x, DirPath = paste(mainDir,"/",subDir, "/", sep = "")))
	    #)
	  }
	  if (length(rankedSpec) > ncandidates) {
	    #try(
	    plots <- lapply(1:length(rankedSpec[1:ncandidates]), function(x) plotCandidatesRC(fmz, frt, highCESpec, DatasetName,
		output, x, DirPath = paste(mainDir,"/",subDir, "/", sep = "")))
	    #)
	  }
	}	else NULL
	...
	pdf(file = paste(mainDir, "/", subDir, "/", "pseudoMSMS_", DatasetName, "_", round(fmz,3), "mz_", 
	round(frt,3), "s", ".pdf", sep=""), width = 8, height = 5)
	# adjust plotting function to work with RAMClustR pseudo-MS/MS object
	plot(pseudoSpec[,1], pseudoSpec[,2], type = 'h', xlim = c(50, max(pseudoSpec[,1]) + 100), 
	ylim = c(0, max(pseudoSpec[,2]) + max(pseudoSpec[,2])/1.5), xlab = "m/z", ylab = "intensity (a.u.)", 
	col = "black", lwd = 1, main = paste("Pseudo-MS/MS Feature:", fmz, "m/z,", frt, "s"), cex.main = 0.95, bty = "L", xaxs = "i", yaxs = "i")
	text(pseudoSpec[,1] - 10, pseudoSpec[,2], as.character(round(pseudoSpec[,1],3)), pos = 4, cex = 0.8, srt = 45)
	dev.off()
	write.csv(pseudoSpec, file = paste(mainDir, "/", subDir, "/", "pseudoMSMS_", DatasetName, "_", round(fmz,3), "mz_", 
   ...
   write.csv(global, file = paste(mainDir, "/", subDir, "/", "Global_Results", ".csv", sep = ""), row.names = FALSE)
	...
	df <- data.frame(targetsTable_file = targetTable, 
	                 ...
	                 libraries = libs, ESImode = ESImode, RTfile = RTfile, 
	                 checkIsotope = checkIsotope, matchWeight = matchWeight, 
	                 tolerance = paste(tolerance, "ppm"), maxMZdiff = paste(maxMZdiff, "Da"),row.names = "Option")
