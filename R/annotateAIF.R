...
annotateAIF <- function(targetTable, filetype = "mzML", libs = "Lipids", ESImode = "POS", RTfile = "none", corThresh = 0.8,
...
if(length(unique(targets[,3])) == 1){
  ...
  if(filetype == "CDF"){
	  ...
	  xcmsF1 <- readMSData(data.path, mode = "onDisk")
	  xcmsF2 <- readMSData(data.path2, mode = "onDisk")
	}
...
for (i in 1:dim(targets)[1]){
	...
	try(
	  specs <- getPseudoMSMS(fmz, frt, xcmsF1, xcmsF2, peaksF1, peaksF2, filetype = filetype, CE = 1,
	                         cthres1 = corThresh, cthres2 = corThresh, plotResults = TRUE, SpName, DirPath)
	)
